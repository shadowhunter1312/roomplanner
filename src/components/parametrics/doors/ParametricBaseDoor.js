...
export class ParametricBaseDoor extends EventDispatcher {
    ...
    __proceedure() {
        ...
        if(doorGeometries.length){
            tempGeometry = BufferGeometryUtils.mergeGeometries([doorGeometries[0]], true);//Maintain all door geometries as same group for material assignment purposes
            if(doorGeometries[1]){
                tempGeometry = BufferGeometryUtils.mergeGeometries([tempGeometry, doorGeometries[1]], true);//Maintain all door geometries as same group for material assignment purposes
            }       
            mergeGeometries.push(tempGeometry);     
        }

        doorGeometry = BufferGeometryUtils.mergeGeometries(mergeGeometries, true);
    ...
    __makeOneDoor(frameWidth, openingDirection, materialId = 1, doorSide = 'Right') {
        ...
        if (this.__handleType !== DOOR_HANDLE_TYPES.None) {
            ...
            aDoorGeometry = BufferGeometryUtils.mergeGeometries([aDoorGeometry, front_handle, back_handle], true);
